<div 
    x-data="{
        ...
        scrollToBottom() {
            // Added a small timeout to ensure the DOM has fully rendered the new messages
            setTimeout(() => {
...
>
    ...
    <div 
        x-ref="chatContainer"
        class="h-[500px] overflow-y-auto p-6 bg-gray-50 space-y-4"
    >
        @forelse($replies as $reply)
            @php 
                $isUser = $reply->user_id === auth()->id();
                $isRequester = $reply->user_id === $ticket->user_id;
            @endphp
            
            <div class="flex {{ $isUser ? 'justify-end' : 'justify-start' }} animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div class="flex items-end gap-2 max-w-[75%] {{ $isUser ? 'flex-row-reverse' : '' }}">
                    {{-- Avatar --}}
        ...
        @empty
            <div class="flex items-center justify-center h-full">
                <div class="text-center">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900">{{ __('creators-ticketing::resources.chat.no_messages_heading') }}</h3>
        ...
        <form wire:submit.prevent="sendMessage" class="flex items-end gap-3">
            <div class="flex-1">
                <textarea 
                    ...
                    rows="3"
                    class="block w-full px-4 py-3 rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 resize-none transition"
            ...
            <button 
                ...
                class="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm hover:shadow-md transition-all duration-150 flex items-center gap-2 font-semibold"
            >
                ...
                <span wire:loading.remove wire:target="sendMessage">{{ __('creators-ticketing::resources.chat.send') }}</span>
                <span wire:loading wire:target="sendMessage">{{ __('creators-ticketing::resources.chat.sending') }}</span>
            </button>
        </form>
        <p class="mt-2 text-xs text-gray-500">{{ __('creators-ticketing::resources.chat.shortcut_hint') }}</p>
    </div>
    @endif
</div>